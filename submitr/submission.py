...
from dcicutils.submitr.progress_constants import (
    PROGRESS_INGESTER,
    PROGRESS_LOADXL,
    PROGRESS_PARSE,
)
...
from submitr.output import (
    PRINT,
    PRINT_OUTPUT,
    PRINT_STDOUT,
    SHOW,
    get_output_file,
    setup_for_output_file_option,
)
...
def set_output_file(output_file):
    if output_file:
        ...
        PRINT, PRINT_OUTPUT, PRINT_STDOUT, SHOW = setup_for_output_file_option(
            output_file
        )
...
PROGRESS_TIMEOUT = (
    60 * 10
)  # ten minutes (note this is for both server validation and submission)
...
def _get_user_record(server, auth, quiet=False):
    ...
    try:
        if (
            user_record_response.status_code in (401, 403)
            and user_record.get("Title") == "Not logged in."
        ):
            if not quiet:
    ...
    if not quiet:
        SHOW(
            f"Portal recognizes you as{' (admin)' if _is_admin_user(user_record) else ''}:"
...
def _get_defaulted_institution(
    institution, user_record, portal=None, quiet=False, verbose=False
):
    """
    ...
    if not institution:
        ...
        if not institution:
            raise SyntaxError(
                "Your user profile has no institution declared,"
...
def _get_defaulted_project(
    project, user_record, portal=None, quiet=False, verbose=False
):
    """
    ...
    if not project:
        ...
        if len(project_roles) == 0:
            ...
            raise SyntaxError(
                "You must use --project to specify which project you are submitting for"
                " (probably one of: %s)."
                % ", ".join([x["project"]["@id"] for x in project_roles])
            )
        else:
...
def _get_defaulted_award(
    award, user_record, portal=None, error_if_none=False, quiet=False, verbose=False
):
    """
    ...
    if not award:
        ...
        if len(lab_awards) == 0:
            ...
            if error_if_none:
                raise SyntaxError(
                    f"Your lab ({lab['@id']}) declares multiple awards."
...
def _get_defaulted_lab(
    lab, user_record, portal=None, error_if_none=False, quiet=False, verbose=False
):
    """
    ...
    if not lab:
        ...
        if not lab:
            if error_if_none:
                raise SyntaxError(
                    "Your user profile has no lab declared,"
...
def _get_defaulted_consortia(
    consortia, user_record, portal=None, error_if_none=False, quiet=False, verbose=False
):
    """
    ...
    if not consortia:
        consortia = [
            consortium.get("@id", None)
            for consortium in user_record.get("consortia", [])
        ]
        if not consortia:
            if error_if_none:
                raise SyntaxError(
                    "Your user profile has no consortium declared,"
    ...
    if portal:
        for consortium in consortia:
            consortium_path = (
                ...
                if not consortium.startswith("/")
                else consortium
            )
            if not (
                consortium_object := portal.get_metadata(
                    consortium_path, raise_exception=False
                )
            ):
...
def _get_defaulted_submission_centers(
    submission_centers,
    user_record,
    portal=None,
    error_if_none=False,
        ...
        if portal:
            if submission_centers := _get_submission_centers(portal):
                ...
                for submission_center in submission_centers:
                    SHOW(
                        f"- {submission_center.get('name')} ({submission_center.get('uuid')})"
                    )
    ...
    if not submission_centers:
        ...
        submission_centers = [
            sc.get("@id", None) for sc in user_record.get("submission_centers", [])
        ]
        submission_centers = list(set(submits_for + submission_centers))
        if not submission_centers:
            if error_if_none:
                raise SyntaxError(
                    "Your user profile has no submission center declared,"
                    " so you must specify --submission-center explicitly."
                )
            SHOW(
                "ERROR: No submission center was inferred. Use the --submission-center option."
            )
    ...
    if portal:
        for submission_center in submission_centers:
            submission_center_path = (
                ...
                if not submission_center.startswith("/")
                else submission_center
            )
            if not (
                submission_center_object := portal.get_metadata(
                    submission_center_path, raise_exception=False
                )
            ):
                ...
                if verbose:
                    annotated_submission_centers.append(
                        f"{submission_center_name} ({submission_center_uuid})"
                    )
                else:
    ...
    if annotated_submission_centers:
        if not quiet:
            SHOW(
                f"Submission center is{suffix}:",
                ", ".join(annotated_submission_centers),
            )
    else:
        if not quiet:
...
def _do_app_arg_defaulting(
    app_args, user_record, portal=None, quiet=False, verbose=False
):
...
def _ingestion_submission_item_url(server, uuid):
    ...
    return (
        url_path_join(server, "ingestion-submissions", uuid)
        + "?format=json&datastore=database"
    )
...
def _analyze_protected_donor_workbook(file_name: Optional[str], portal):
    """Analyze a workbook using dcicutils' ProtectedDonor public API."""
    if not _is_excel_workbook(file_name):
        return None
    try:
        workbook = openpyxl.load_workbook(file_name)
    except Exception:
        # Let normal parsing/validation report malformed or unsupported workbooks.
        return None
...
def _prepare_protected_donor_transform(
    *,
    ...
    if analysis.invalid_references or analysis.missing_references:
        ...
        workbook = openpyxl.load_workbook(ingestion_filename)
...
def _ensure_protected_donor_transformed_workbook(
    ...
    if not os.path.exists(transformed_workbook_path):
        ...
        PRINT("Check that the protected-item sheets reference a Donor with a submitted_id column.")
        sys.exit(1)
...
def _initiate_server_ingestion_process(
    ...
    if isinstance(validation_ingestion_submission_object, dict):
        ...
        validation_ingestion_submission_uuid = (
            validation_ingestion_submission_object.get("uuid", None)
        )
        if not (
            isinstance(
                validation_parameters := validation_ingestion_submission_object.get(
                    "parameters"
                ),
                dict,
            )
    ...
    submission_post_data = {
        ...
        "merge": merge,
        "ingestion_directory": (
            os.path.dirname(ingestion_filename) if ingestion_filename else None
        ),
    ...
    if is_resume_submission and validation_ingestion_submission_object:
        if validation_parameters := validation_ingestion_submission_object.get(
            "parameters"
        ):
            submission_post_data["validation_datafile"] = validation_parameters.get(
                "datafile"
            )
            submission_post_data["ingestion_directory"] = validation_parameters.get(
                "ingestion_directory"
            )

    response = _post_submission(
        portal=portal,
        ...
        is_server_validation=is_server_validation,
        debug=debug,
    )
    ...
    if validation_ingestion_submission_uuid and validation_parameters:
        ...
        portal.patch_metadata(
            object_id=validation_ingestion_submission_uuid, data=validation_parameters
        )
...
def _post_submission(
    ...
    if isinstance(add_submission_center, str) and (
        add_submission_center not in creation_submission_centers
    ):
    ...
    creation_response = portal.post(
        creation_post_url, json=creation_post_data, raise_for_status=True
    )
    ...
    new_style_submission_url = url_path_join(
        portal.server, submission_id, "submit_for_ingestion"
    )
    if debug:
        if is_server_validation:
            ...
            PRINT(
                f"DEBUG: Initiating server submission process after server validation timeout."
            )
        else:
    ...
    if debug:
        PRINT(
            f"DEBUG: Initiated server {'validation' if is_server_validation else 'submission'} process."
        )
    return response.json()


def _post_files_data(
    submission_protocol, ingestion_filename
) -> Dict[Literal["datafile"], Optional[BinaryIO]]:
    """
...
def _resolve_app_args(
    ...
    if app == APP_CGAP:
        ...
        def splitter(x):
            return (
                None
                if x is None
                else [y for y in [x.strip() for x in x.split(",")] if y]
            )

        consortia = None if consortium is None else splitter(consortium)
        submission_centers = (
            None if submission_center is None else splitter(submission_center)
        )
        required_args = {
            "consortia": consortia,
            "submission_centers": submission_centers,
    ...
    if extra_keys:
        raise ValueError(
            there_are(
                extra_keys,
                kind="inappropriate argument",
                joiner=conjoined_list,
                punctuate=True,
            )
        )
...
def submit_any_ingestion(
    ...
    *,
    ...
    if not portal.ping():
        SHOW(
            f"Portal credentials do not seem to work: {portal.keys_file} ({env}). "
    ...
    metadata_bundles_bucket = get_metadata_bundles_bucket_from_health_path(
        key=portal.key
    )
    if not _do_app_arg_defaulting(
        app_args, user_record, portal, quiet=json_only and not verbose, verbose=verbose
    ):
        pass
    ...
    autoadd = {}
    if app_args and isinstance(
        submission_centers := app_args.get("submission_centers"), list
    ):
        ...
        if not isinstance(consortia := app_args.get("consortia"), list):
            consortia = (
                [consortia]
                if isinstance(consortia := app_args.get("consortia"), str)
                else []
            )
        if len(consortia) == 1:
        ...
        if len(submission_centers) == 1:
            autoadd["submission_centers"] = [
                extract_identifying_value_from_path(submission_centers[0])
            ]
        elif len(submission_centers) > 1:
            PRINT(f"Multiple submission centers: {', '.join(submission_centers)}")
            PRINT(
                f"You must specify only one submission center using the --submission-center option."
            )
            sys.exit(1)
    ...
    if add_submission_center:
        if not _is_admin_user(user_record):
            PRINT(
                "ERROR: Cannot use the --add-submission-center if you are not an admin user."
            )
            sys.exit(1)
        found_submission_centers = [
            submission_center
            for submission_center in known_submission_centers
            if (submission_center.get("name") == add_submission_center)
            or (submission_center.get("uuid") == add_submission_center)
        ]
        if not found_submission_centers or not found_submission_centers[0].get("uuid"):
            PRINT(
                f"ERROR: Specified submission center ({add_submission_center}) not found. Use one of:"
            )
            for known_submission_center in known_submission_centers:
                PRINT(
                    f"- {known_submission_center.get('name')} ({known_submission_center.get('uuid')})"
                )
            sys.exit(1)
    ...
    if _is_admin_user(user_record):
        ...
        valid_submission_centers = ",".join(
            [item["name"] for item in known_submission_centers]
        )
        ...
        for submission_center in submission_centers:
            for known_submission_center in known_submission_centers:
                if (
                    f"/submission-centers/{known_submission_center['uuid']}/"
                    == submission_center
                ):
                    valid_submission_centers.append(known_submission_center["name"])
        valid_submission_centers = ",".join(
            [submission_center for submission_center in valid_submission_centers]
        )
    else:
        valid_submission_centers = ""
    ...
    if not validate_remote_only and not validate_local_skip:
        structured_data = _validate_locally(
            ingestion_filename,
            portal,
            validation=validation,
    ...
    if not validate_local_only and not validate_remote_skip:
        ...
        server_validation_done, server_validation_status, server_validation_response = (
            _monitor_ingestion_process(
                validation_uuid,
                portal.server,
                portal.env,
                app=portal.app,
                keys_file=portal.keys_file,
        ...
        PRINT(
            "Skipping remote (server) validation (as requested via"
    ...
    submission_done, submission_status, submission_response = (
        _monitor_ingestion_process(
            submission_uuid,
            portal.server,
            portal.env,
...
def _get_recent_submissions(
    portal: Portal, count: int = 30, name: Optional[str] = None
) -> List[dict]:
...
def _print_recent_submissions(
    ...
    if mine:
        try:
            user_record = _get_user_record(
                portal.server, auth=portal.key_pair, quiet=True
            )
    ...
    if submissions := _get_recent_submissions(portal, count, name=user_name):
        ...
        for submission in submissions:
            ...
            line = (
                f"{submission_uuid}: {format_datetime(submission_created, notz=True)}"
            )
            ...
            if details:
                line_detail = ""
                if submitted_by := submission.get("submitted_by", {}).get(
                    "display_title"
                ):
                    if line_detail:
                        line_detail += " | "
                ...
                if (submission_params := submission.get("parameters")) and (
                    ...
                    if submission_file == "null":
                        if validation_datafile := submission_params.get(
                            "validation_datafile"
                        ):
        ...
        if not verbose:
            ...
            print_boxed(
                lines,
                right_justified_macro=(
                    "[COUNT]",
                    lambda: f"Showing: {len(submissions)}",
                ),
            )
        return True
    return False
...
def _monitor_ingestion_process(
    uuid: str,
    server: str,
    env: str,
    ...
    def define_progress_callback(
        ...
        def progress_report(status: dict) -> None:  # noqa
            ...
            ingester_parse_started = status.get(
                PROGRESS_INGESTER.PARSE_LOAD_INITIATE, None
            )
            ...
            ingester_validate_started = status.get(
                PROGRESS_INGESTER.VALIDATE_LOAD_INITIATE, None
            )
            ...
            loadxl_started_second_round = status.get(
                PROGRESS_LOADXL.START_SECOND_ROUND, None
            )
            ...
            ingestion_message = (
                status.get("loadxl_message_verbose", "")
                if verbose
                else status.get("loadxl_message", "")
            )
            ...
            loadxl_phase = (
                ...
                else (1 if loadxl_started is not None else 0)
            )
    ...
    def interrupt_exit_message(bar: ProgressBar):
        ...
        SHOW(
            f"Your {'validation' if validation else 'submission'} is still running on the server: {uuid}"
        )
        SHOW(f"Use this command to check its status: {command_summary}")

    if not (uuid_metadata := portal.get_metadata(uuid, raise_exception=False)) and not (
        uuid_metadata := lookup_file_metadata_by_file_name(uuid, portal)
    ):  # noqa
        found_non_suffixed_uuid = False
        if (dot := uuid.find(".")) > 0:
            if uuid_metadata := portal.get_metadata(
                uuid := uuid[:dot], raise_exception=False
            ):
                found_non_suffixed_uuid = True
        if not found_non_suffixed_uuid:
            message = (
                f"Submission ID not found: {uuid}"
                if uuid != "dummy"
                else "No submission ID specified."
            )
            message += (
    ...
    if not portal.is_schema_type(uuid_metadata, INGESTION_SUBMISSION_TYPE_NAME):
        ...
        raise Exception(
            f"Given ID is not for a submission or validation: {uuid} ({undesired_type})"
    ...
    if validation:
        SHOW(
            f"Waiting{f' (up to about {PROGRESS_TIMEOUT}s)' if verbose else ''}"
    ...
    started = time.time()
    progress = define_progress_callback(
        PROGRESS_MAX_CHECKS,
    ...
    for n in range(PROGRESS_MAX_CHECKS):
        ...
        if (most_recent_get_ingestion_status_time is None) or (
            (time.time() - most_recent_get_ingestion_status_time)
            >= PROGRESS_GET_INGESTION_STATUS_INTERVAL
        ):  # noqa
            ingestion_status = portal.get(f"/ingestion-status/{uuid}")
            if (ingestion_status.status_code == 200) and (
                ingestion_status := ingestion_status.json()
            ):
                loadxl_done = (
                    ingestion_status.get(PROGRESS_LOADXL.DONE, None) is not None
                )
            else:
                ingestion_status = {}
                loadxl_done = False
        ...
        if (
            loadxl_done
            or (most_recent_get_ingestion_submission_time is None)
            or (
                (time.time() - most_recent_get_ingestion_submission_time)
                >= PROGRESS_GET_INGESTION_SUBMISSION_INTERVAL
            )
        ):  # noqa
            if most_recent_get_ingestion_submission_time is None:
                ...
                progress(
                    {
                        "check_server": True,
                        "status": (check_status or "unknown").title(),
                        **ingestion_status,
                    }
                )
            # Do the actual portal check here (i.e by fetching the IngestionSubmission object)..
            [check_done, check_status, check_response] = _check_ingestion_progress(
                uuid, keypair=portal.key_pair, server=portal.server
            )
            if check_done:
                break
    ...
    if not check_done:
        ...
        SHOW(
            f"Timed out (after {format_duration(round(time.time() - started))}) WAITING for {action}."
        )
        SHOW(
            f"Your {'validation' if validation else 'submission'} is still running on the server: {uuid}"
        )
    ...
    if (
        check_submission_script
        and check_response
        and (check_parameters := check_response.get("parameters", {}))
        and tobool(check_parameters.get("validate_only"))
        and not check_parameters.get("submission_uuid")
    ):  # noqa
        ...
        validation_info = check_response.get("additional_data", {}).get(
            "validation_output"
        )
        # TODO: Cleanup/unify error structure from client and server!
        if isinstance(validation_info, list):
            validation_errors = [
                item for item in validation_info if item.lower().startswith("errored")
            ]
            if validation_errors:
                PRINT("\nServer validation errors were encountered for this metadata.")
                PRINT(
                    "You will need to correct any errors and resubmit via submit-metadata-bundle."
                )
                sys.exit(1)
        ...
        PRINT(f"Validation results (server): OK {chars.check}")
        if not yes_or_no(
            "Do you want to now continue with the submission for this metadata?"
        ):
        ...
        if debug:
            PRINT(
                "DEBUG: Continuing with submission process after a previous server validation timeout."
            )
        submission_uuid = _initiate_server_ingestion_process(
            portal=portal,
    ...
    if check_submission_script or verbose or debug:  # or not validation
        PRINT(
            f"Details for this server {'validation' if validation else 'submission'} ({uuid}) below:"
        )
        _print_submission_summary(
            portal,
            check_response,
            nofiles=nofiles,
            check_submission_script=check_submission_script,
            verbose=verbose,
            debug=debug,
        )
    ...
    if check_status != "success":
        PRINT(
            f"{'Validation' if validation else 'Submission'} results (server): ERROR"
            f"{f' ({check_status})' if check_status not in ['failure', 'error'] else ''}"
        )
        printed_newline = False
        if check_response and (
            additional_data := check_response.get("additional_data")
        ):
            if validation_info := additional_data.get("validation_output"):
                if isinstance(validation_info, list):
                    if errors := [
                        ...
                        if info.lower().startswith("error:")
                    ]:
                        ...
                        for error in errors:
                            PRINT_OUTPUT(
                                f"- {_format_server_error(error, indent=2, debug=debug)}"
                            )
                elif isinstance(validation_info, dict):
                    if (
                        (validation_errors := validation_info.get("validation"))
                    ...
                    if debug:
                        ...
                        ref_errors = validation_info.get("ref")
                    if ref_errors and (
                        ref_errors := _validate_references(
                            ref_errors, None, debug=debug
                        )
                    ):
                        if not printed_newline:
                            PRINT_OUTPUT()
                            printed_newline = True
                        _print_reference_errors(
                            ref_errors, verbose=verbose, debug=debug
                        )
        if (
            check_response
            and isinstance(other_errors := check_response.get("errors"), list)
            and other_errors
        ):
            if not printed_newline:
                PRINT_OUTPUT()
        ...
        if output_file := get_output_file():
            PRINT_STDOUT(
                f"Exiting with server validation errors; see your output file: {output_file}"
            )
        else:
...
def _summarize_submission(
    uuid: str, app: str, server: Optional[str] = None, env: Optional[str] = None
):
    if env:
...
def _print_submission_summary(
    ...
    def is_admin_user(user_record: Optional[dict]) -> bool:  # noqa
        if (
            ...
            or not user_record
            or not (user_uuid := user_record.get("uuid"))
        ):
            return None
        try:
    ...
    if submission_parameters := result.get("parameters", {}):
        ...
        if submission_file := submission_parameters.get("datafile"):
            if submission_file == "null":
                ...
                if validation_datafile := submission_parameters.get(
                    "validation_datafile"
                ):
    ...
    if submission_parameters:
        extra_file_info = ""
        if (
            datafile_size := submission_parameters.get("datafile_size", None)
        ) is not None:
            if (
                not isinstance(datafile_size, int)
                and isinstance(datafile_size, str)
                and datafile_size.isdigit()
            ):
    ...
    if validation:
        lines.append(
            f"Validation Only: Yes {chars.larrow} {chars.larrow} {chars.larrow}"
        )
        if submission_parameters and (
            associated_submission_uuid := submission_parameters.get("submission_uuid")
        ):
            lines.append(f"Associated Submission ID: {associated_submission_uuid}")
    elif submission_parameters and (
        associated_validation_uuid := submission_parameters.get("validation_uuid")
    ):
        lines.append(
            f"Associated Validation ID:"
    ...
    if submitted_by := result.get("submitted_by", {}).get("display_title"):
        ...
        if consortia:
            if submission_center:
                lines.append(
                    f"Submitted By: {submitted_by} ({consortium} | {submission_center})"
                )
            else:
    ...
    if additional_data := result.get("additional_data"):
        if (validation_info := additional_data.get("validation_output")) and isinstance(
            validation_info, dict
        ):
            # TODO: Cleanup/unify error structure from client and server!
            if ref_errors := _validate_references(validation_info.get("ref"), None):
                errors.extend(
                    _format_reference_errors(ref_errors, verbose=verbose, debug=debug)
                )
    ...
    if processing_status := result.get("processing_status"):
        ...
        if additional_data := result.get("additional_data"):
            if (
                validation_info := additional_data.get("validation_output")
            ) and isinstance(validation_info, list):
                if status := [
                    info
                    for info in validation_info
                    if info.lower().startswith("status:")
                ]:
    ...
    if additional_data := result.get("additional_data"):
        if (validation_info := additional_data.get("validation_output")) and isinstance(
            ...
            summary_lines = []
            if types := [
                info for info in validation_info if info.lower().startswith("types")
            ]:
                summary_lines.append(types[0])
            if created := [
                info for info in validation_info if info.lower().startswith("created")
            ]:
                summary_lines.append(created[0])
            if updated := [
                info for info in validation_info if info.lower().startswith("updated")
            ]:
                summary_lines.append(updated[0])
            if skipped := [
                info for info in validation_info if info.lower().startswith("skipped")
            ]:
                summary_lines.append(skipped[0])
            if checked := [
                info for info in validation_info if info.lower().startswith("checked")
            ]:
                summary_lines.append(checked[0])
            if errored := [
                info for info in validation_info if info.lower().startswith("errored")
            ]:
                summary_lines.append(errored[0].replace("Errored", "Errors"))
            if errors := [
                info for info in validation_info if info.lower().startswith("error:")
            ]:
                pass
            if total := [
                info for info in validation_info if info.lower().startswith("total")
            ]:
    ...
    if validation_info:
        summary_lines = []
        if s3_data_file := [
            info for info in validation_info if info.lower().startswith("s3 file: ")
        ]:
            s3_data_file = s3_data_file[0][9:]
            if (rindex := s3_data_file.rfind("/")) > 0:
                s3_data_bucket = (
                    s3_data_file[5:rindex] if s3_data_file.startswith("s3://") else ""
                )
                s3_data_file = s3_data_file[rindex + 1:]
                if s3_data_bucket:
                    if len(s3_data_bucket_parts := s3_data_bucket.split("/")) == 2:
                        summary_lines.append(
                            f"AWS S3 Bucket: {s3_data_bucket_parts[0]}"
                        )
        ...
        if s3_details_file := [
            info for info in validation_info if info.lower().startswith("details: ")
        ]:
            s3_details_file = s3_details_file[0][9:]
            if (rindex := s3_details_file.rfind("/")) > 0:
                s3_details_bucket = (
                    s3_details_file[5:rindex]
                    if s3_details_file.startswith("s3://")
                    else ""
                )
    ...
    if additional_data and not nofiles:
        if upload_files := additional_data.get("upload_info"):
            ...
            for upload_file in upload_files:
                ...
                if verbose:
                    upload_file_accession_name, upload_file_type = (
                        _get_upload_file_info(portal, upload_file_uuid)
                    )
                else:
                ...
                if upload_file_accession_name:
                    lines.append(
                        f"Upload File Accession ID: {upload_file_accession_name}"
                    )
                if upload_file_type:
    ...
    if lines:
        lines = (
            [
                "===",
                f"SMaHT {'Validation' if validation else 'Submission'} Summary"
                f"{f' ({submitr_version})' if submitr_version else ''} [UUID]",
                "===",
            ]
            + lines
            + ["==="]
...
def _print_upload_file_summary(portal: Portal, file_object: dict) -> None:
    ...
    file_modified = (
        format_datetime(file_object.get("last_modified", {}).get("date_modified")) or ""
    )
    file_modified_by = (
        file_object.get("last_modified", {}).get("modified_by", {}).get("display_title")
        or ""
    )
    submission_centers = (
        _format_affiliation(file_object.get("submission_centers")) or ""
    )
    ...
    ingestion_submission_uuid = lookup_ingestion_submission_from_upload_file(
        portal, file_name
    )
    lines = [
        "===",
        ...
        (
            f"Associated Submission ID: {ingestion_submission_uuid}"
            if ingestion_submission_uuid
            else None
        ),
        f"===",
        (
            f"Submitted: {file_created} | {file_submitted_by}"
            if file_created and file_submitted_by
            else None
        ),
        (
            f"Modified: {file_modified} | {file_modified_by}"
            if file_modified and file_modified_by
            else None
        ),
...
def _get_upload_file_info(
    portal: Portal, uuid: str
...
def _fetch_submission_results(
    metadata_bundles_bucket: str, uuid: str
) -> Optional[Tuple[str, dict]]:
    return _fetch_results(metadata_bundles_bucket, uuid, "submission.json")


def _fetch_exception_results(
    metadata_bundles_bucket: str, uuid: str
...
def _fetch_results(
    metadata_bundles_bucket: str, uuid: str, file: str
) -> Optional[Tuple[str, str]]:
    ...
    try:
        ...
        if (
            hasattr(e, "response")
            and e.response.get("Error", {}).get("Code", "").lower() == "accessdenied"
        ):
...
def _validate_locally(
    ...
    autoadd: Optional[dict] = None,
    validation: bool = False,
    validate_local_only: bool = False,
    submit_update: bool = False,
            ...
            if status.get(PROGRESS_PARSE.LOAD_START):
                ...
                if nrows > 0:
                    ...
                    if nsheets > 0:
                        ...
                        PRINT(
                            f"Parsing metadata file which has a total of {nrows} row{'s' if nrows != 1 else ''}."
                        )
                elif nsheets > 0:
                    PRINT(
                        f"Parsing metadata file which has {nsheets} sheet{'s' if nsheets != 1 else ''}."
                    )
                else:
                    PRINT(
                        f"Parsing metadata file which has a total of {nrows} row{'s' if nrows != 1 else ''}."
                ...
                return
            elif status.get(PROGRESS_PARSE.LOAD_ITEM) or status.get(
                PROGRESS_PARSE.LOAD_DONE
            ):
                ...
                nrefs_unresolved = (
                    status.get(PROGRESS_PARSE.LOAD_COUNT_REFS_NOT_FOUND) or 0
                )
                nrefs_lookup = status.get(PROGRESS_PARSE.LOAD_COUNT_REFS_LOOKUP) or 0
                nrefs_exists_cache_hit = (
                    status.get(PROGRESS_PARSE.LOAD_COUNT_REFS_EXISTS_CACHE_HIT) or 0
                )
                nrefs_lookup_cache_hit = (
                    status.get(PROGRESS_PARSE.LOAD_COUNT_REFS_LOOKUP_CACHE_HIT) or 0
                )
    ...
    if debug:
        ...
        PRINT_OUTPUT(
            f"DEBUG: Reference total found count: {structured_data.ref_total_found_count}"
        )
        PRINT_OUTPUT(
            f"DEBUG: Reference total not found count: {structured_data.ref_total_notfound_count}"
        )
        PRINT_OUTPUT(
            f"DEBUG: Reference exists cache hit count: {structured_data.ref_exists_cache_hit_count}"
        )
        PRINT_OUTPUT(
            f"DEBUG: Reference exists cache miss count: {structured_data.ref_exists_cache_miss_count}"
        )
        PRINT_OUTPUT(
            f"DEBUG: Reference exists internal count: {structured_data.ref_exists_internal_count}"
        )
        PRINT_OUTPUT(
            f"DEBUG: Reference exists external count: {structured_data.ref_exists_external_count}"
        )
        PRINT_OUTPUT(
            f"DEBUG: Reference lookup cache hit count: {structured_data.ref_lookup_cache_hit_count}"
        )
        PRINT_OUTPUT(
            f"DEBUG: Reference lookup cache miss count: {structured_data.ref_lookup_cache_miss_count}"
        )
        PRINT_OUTPUT(
            f"DEBUG: Reference lookup count: {structured_data.ref_lookup_count}"
        )
        PRINT_OUTPUT(
            f"DEBUG: Reference lookup found count: {structured_data.ref_lookup_found_count}"
        )
        PRINT_OUTPUT(
            f"DEBUG: Reference lookup not found count: {structured_data.ref_lookup_notfound_count}"
        )
        PRINT_OUTPUT(
            f"DEBUG: Reference lookup error count: {structured_data.ref_lookup_error_count}"
        )
        PRINT_OUTPUT(
            f"DEBUG: Reference invalid identifying property count:"
    ...
    if validation_okay:
        ...
        if output_file:
            PRINT_STDOUT(
                f"Exiting with preliminary validation errors; see your output file: {output_file}"
            )
        else:
            if not verbose:
                PRINT_STDOUT()
    ...
    if verbose:
        ...
        PRINT(
            "Skipping analysis of metadata wrt creates/updates to be done (per --noanalyze)."
        )
...
def _validate_data(
    ...
    if not ignore_orphans:
        if (
            unreferenced_error_count := report_unreferenced_references(
                structured_data, printf=PRINT_OUTPUT
            )
        ) > 0:
...
def _validate_references(
    ref_errors: Optional[List[dict]], ingestion_filename: str, debug: bool = False
) -> List[str]:
    def normalize(ref_errors: Optional[List[dict]]) -> None:  # noqa
        ...
        if isinstance(ref_errors, list):
            for ref_error in ref_errors:
                if isinstance(src := ref_error.get("src"), dict) and isinstance(
                    file := src.get("file"), str
                ):
    ...
    if isinstance(ref_errors, list):
        for ref_error in ref_errors:
            if debug:
                ref_validation_errors.append(
                    f"{_format_issue(ref_error, ingestion_filename)}"
                )
            elif ref_error.get("truncated") is True:
                ref_validation_errors_truncated = {
                    "ref": f"{_format_issue(ref_error, ingestion_filename)}"
                }
                ...
                if ref := ref_error.get("error"):
                    if ref_error_existing := [
                        r for r in ref_validation_errors if r.get("ref") == ref
                    ]:
    ...
    if debug:
        ...
        ref_validation_errors = sorted(
            ref_validation_errors, key=lambda item: item.get("ref")
        )
...
def _print_reference_errors(
    ref_errors: List[dict], verbose: bool = False, debug: bool = False
) -> None:
    if errors := _format_reference_errors(
        ref_errors=ref_errors, verbose=verbose, debug=debug
    ):
        for error in errors:
            PRINT_OUTPUT(error)


def _format_reference_errors(
    ref_errors: List[dict], verbose: bool = False, debug: bool = False
) -> List[str]:
    errors = []
    if isinstance(ref_errors, list) and ref_errors:
        nref_errors = len(
            [
                ...
                or (not r.get("ref", "").startswith("Truncated"))
            ]
        )
            ...
            for ref_error in ref_errors:
                if ref_error["ref"].startswith("Truncated"):
                    ...
                    if isinstance(srcs := ref_error.get("srcs"), list):
                        ...
                        final_src_lines = _collapse_formatted_srcs(
                            formatted_srcs, verbose
                        )
...
def _validate_initial(structured_data: StructuredDataSet, portal: Portal) -> List[str]:
    ...
    for schema_name in structured_data.data:
        if schema_data := portal.get_schema(schema_name):
            if identifying_properties := schema_data.get(
                EncodedSchemaConstants.IDENTIFYING_PROPERTIES
            ):
                ...
                if data := structured_data.data[schema_name]:
                    ...
                    if not data_properties & identifying_properties:
                        ...
                        initial_validation_errors.append(
                            f"No identifying properties for type: {schema_name}"
                        )
            if required_properties := schema_data.get(JsonSchemaConstants.REQUIRED):
                ...
                if data := structured_data.data[schema_name]:
                    ...
                    if (data_properties & required_properties) != required_properties:
                        if (
                            missing_required_properties := required_properties
                            - data_properties
                        ):
                            # Missing required properties for this object.
                            for (
                                missing_required_property
                            ) in missing_required_properties:
                                initial_validation_errors.append(
...
def _print_structured_data_verbose(
    portal: Portal,
    structured_data: StructuredDataSet,
    ingestion_filename: str,
    upload_folder: str,
    ...
    if structured_data.data:
        ...
        for type_name in sorted(structured_data.data):
            PRINT_OUTPUT(
                f"  - {type_name}: {len(structured_data.data[type_name])}"
...
def _print_structured_data_status(
    portal: Portal,
    structured_data: StructuredDataSet,
    validation: bool = False,
    submit_update: bool = False,
    ...
    def define_progress_callback(debug: bool = False) -> None:
        ...
        nlookups = 0
        bar = ProgressBar(
            nobjects, "Calculating", interrupt_exit=True, interrupt_message="analysis"
        )
        ...
        def progress_report(status: dict) -> None:  # noqa
            ...
            if status.get(PROGRESS_PARSE.ANALYZE_START):
                ...
                PRINT(
                    f"Analyzing metadata file which has {ntypes} type{'s' if ntypes != 1 else ''}"
                ...
                if not object_info.diffs:
                    ...
                    for diff_path in object_info.diffs:
                        if (diff := object_info.diffs[diff_path]).creating_value:
                            ...
                            PRINT(
                                f"     UPDATE {diff_path}: {diff.updating_value} -> {diff.value}"
                            )
    ...
    if do_not_continue_because_updates_and_not_submit_update:
        PRINT(
            f"{chars.rarrow} This submission would result in UPDATES to existing data."
        )
        PRINT(f"{chars.rarrow} But you have NOT specified the --submit-update option.")
        PRINT(
            f"{chars.rarrow} This submission will NOT continue. Exiting with no action."
        )
        PRINT()
        exit(0)
...
def _define_portal(
    ...
    def sanity_check_keys_file(keys_file: Optional[str] = None) -> Optional[str]:
        ...
        if keys_file:
            if not keys_file.endswith(".json"):
                PRINT(
                    f"ERROR: The specified keys file{' (from SMAHT_KEYS)' if keys_file_from_env else ''}"
    ...
    try:
        ...
        portal = Portal(
            key or keys_file, env=env, server=server, app=app, raise_exception=True
        )
    except Exception as e:
        if "not found in keys-file" in str(e):
            if not env:
                PRINT(
                    f"No environment specified from keys file: {keys_file or get_default_keys_file()}"
                )
                PRINT(
                    f"Use the --env option with a name from that file; or set your SMAHT_ENV environment variable."
                )
            else:
                PRINT(
                    f"Environment ({env}) not found in keys file: {keys_file or get_default_keys_file()}"
                )
            sys.exit(1)
        else:
            PRINT(e)
    ...
    if report:
        message = (
            f"SMaHT submitr version: {get_version()}"
                ...
                try:
                    with io.open(portal.keys_file) as f:
                        ...
                        if env := [
                            k
                            for k in keys
                            if Portal._normalize_server(keys[k].get("server")) == server
                        ]:
                            portal._env = env[0]
                except Exception:
                    pass
        PRINT(
            f"Portal environment (in keys file) is: {portal.env}{' (from SMAHT_ENV)' if env_from_env else ''}"
        )
        PRINT(f"Portal keys file is: {format_path(portal.keys_file)}")
        portal_version = portal.get_version()
        PRINT(
            f"Portal server is: {portal.server}{f' ({portal_version})' if portal_version else ''}"
        )
...
def _get_consortia(portal: Portal) -> List[str]:
    ...
    if consortia := portal.get_metadata("/consortia?limit=1000"):
        consortia = sorted(
            consortia.get("@graph", []), key=lambda key: key.get("identifier")
        )
        for consortium in consortia:
            if (consortium_name := consortium.get("identifier")) and (
                consortium_uuid := consortium.get("uuid")
            ):  # noqa
...
def _get_submission_centers(portal: Portal) -> List[str]:
    ...
    if submission_centers := portal.get_metadata("/submission-centers?limit=1000"):
        submission_centers = sorted(
            submission_centers.get("@graph", []), key=lambda key: key.get("identifier")
        )
        for submission_center in submission_centers:
            if (submission_center_name := submission_center.get("identifier")) and (
                submission_center_uuid := submission_center.get("uuid")
            ):  # noqa
                results.append(
                    {"name": submission_center_name, "uuid": submission_center_uuid}
                )
    return results


def _get_submission_center(portal: Portal, submission_center: str) -> Optional[str]:
    given_submission_center = os.path.basename(
        submission_center
    )  # just in case it is /submission-centers/{uuid}
    for submission_center in _get_submission_centers(portal):
        if (given_submission_center == submission_center["name"]) or (
            given_submission_center == submission_center["uuid"]
        ):  # noqa
            return submission_center["name"]
    return None


def _print_metadata_file_info(
    file: str,
    env: str,
    refs: bool = False,
    files: bool = False,
    ...
    if (refs is True) or (files is True):
        ...
        if refs is True:
            ...
            def print_refs(
                ...
                def note_output():  # noqa
                    ...
                    if noutput >= max_output and output_file and not truncated:
                        PRINT_STDOUT(
                            f"+ Truncated results | See your output file for full listing: {output_file}"
                        )
                        printf = PRINT_OUTPUT
                        truncated = True
                ...
                for ref in sorted(refs, key=lambda ref: ref["path"]):
                    ...
                    if verbose:
                        srcs = sorted(
                            ref["srcs"],
                            key=lambda src: f"{src['type']}|{src['column']}|"
            ...
            if structured_data.portal and (
                unresolved_refs := [
                    ref
                    for ref in unchecked_refs
    ...
    if not (refs is True):
        if not (files is True):
            PRINT(
                "Note: Use --refs to view references; and --files to view files for upload."
            )
        else:
    ...
    if this_metadata_template_version:
        if this_metadata_template_version == current_metadata_template_version:
            PRINT(
                f"Based on the latest HMS metadata template: {current_metadata_template_version} {chars.check}"
            )
        else:
            print_metadata_version_warning(
                this_metadata_template_version, current_metadata_template_version
