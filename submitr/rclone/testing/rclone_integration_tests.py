...
def test_local_to_amazon():
    ...
    _test_local_to_amazon(AmazonTestEnv.credentials(), use_temporary_credentials=True)
    _test_local_to_amazon(AmazonTestEnv.credentials(), use_temporary_credentials=True, nokms=True)

    _test_local_to_amazon(AmazonTestEnv.credentials(), use_temporary_credentials_key_specific=True)
    _test_local_to_amazon(AmazonTestEnv.credentials(), use_temporary_credentials_key_specific=True, nokms=True)
...
def test_google_to_local():
    ...
    with temporary_test_file() as (tmp_test_file_path, tmp_test_file_name):
        rclone = RClone(destination=config)
        # TODO
        command = rclone.copy(tmp_test_file_path, GoogleTestEnv.bucket, dryrun=True)
        rclone.copy(tmp_test_file_path, GoogleTestEnv.bucket)
        with temporary_file() as tmp_downloaded_file_path:
            gcs = Gcs(credentials)
            gcs.download_file(GoogleTestEnv.bucket, tmp_test_file_name, tmp_downloaded_file_path)
            assert are_files_equal(tmp_test_file_path, tmp_downloaded_file_path) is True
        # TODO
