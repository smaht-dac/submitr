...
def test_integration_testing_connectivity(capsys):
    ...
    assert env_amazon.non_rclone().bucket_exists(env_amazon.bucket) is True
    ...
    assert env_google.non_rclone().bucket_exists(env_google.bucket) is True
...
def __test_rclone_google_to_amazon(env_amazon: EnvAmazon,
    ...
    with Env.temporary_test_file() as (tmp_test_file_path, tmp_test_file_name):
        ...
        cleanup_amazon_file(env_amazon, env_amazon.bucket, key_amazon)
        rclone_amazon = create_rclone_target_amazon(credentials_amazon, bucket=env_amazon.bucket)  # xyzzy-new
        # rclone_amazon.bucket = env_amazon.bucket # xyzzy-delete
...
def _test_rclone_amazon_to_google(env_amazon: EnvAmazon, env_google: EnvGoogle) -> None:
    ...
    with Env.temporary_test_file() as (tmp_test_file_path, tmp_test_file_name):
        ...
        # bucket specified within the RCloneGoogle object (new: 2024-05-10).
        cleanup_google_file(env_google, env_google.bucket, key_google)
        rclone_google = create_rclone_target_google(credentials_google, bucket=env_google.bucket)  # xyzzy-new
        # rclone_google.bucket = env_google.bucket # xyzzy-delete
        rcloner = create_rcloner(source=rclone_amazon, destination=rclone_google)
        if cloud_path.has_separator(key_google):
