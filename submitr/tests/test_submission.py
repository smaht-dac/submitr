...
from ..submission import (  # noqa
    ...
    _get_defaulted_institution,
    _get_defaulted_project,
    _get_defaulted_consortia,
    _get_defaulted_submission_centers,
    _get_user_record,
    _ingestion_submission_item_url,
    _resolve_server,
    resume_uploads,
    ...
    _do_app_arg_defaulting,
    _check_ingestion_progress,  # noQA - again, testing a protected member
    _get_defaulted_lab,
    _get_defaulted_award,
    SubmissionProtocol,
    GENERIC_SCHEMA_TYPE,
    DEFAULT_APP,
    _summarize_submission,
    _collapse_consecutive_ints,
    _collapse_formatted_srcs,
    _format_src,
    _format_issue,
)
...
SOME_SERVER = (
    "http://localhost:7777"  # Dependencies force this to be out of alphabetical order
)
...
SOME_UPLOAD_INFO_RESULT = {"additional_data": {"upload_info": SOME_UPLOAD_INFO}}
...
def make_user_record(title=SOME_USER_TITLE, contact_email=SOME_USER_EMAIL, **kwargs):
    user_record = {"title": title, "contact_email": contact_email, "groups": ["admin"]}
...
def test_get_user_record():
    ...
    def make_mocked_get(auth_failure_code=400):
        def mocked_get(url, *, auth, **kwargs):
            ...
            if auth != SOME_AUTH:
                return FakeResponse(
                    status_code=auth_failure_code, json={"Title": "Not logged in."}
                )
            return FakeResponse(
                status_code=200,
                json={"title": SOME_USER_TITLE, "contact_email": SOME_USER_EMAIL},
            )

        return mocked_get

    with mock.patch(
        "requests.get", return_value=FakeResponse(401, content='["not dictionary"]')
    ):
...
def test_get_defaulted_institution():
    ...
    assert (
        _get_defaulted_institution(
            institution=SOME_INSTITUTION, user_record="does-not-matter"
        )
        == SOME_INSTITUTION
    )
    assert (
        _get_defaulted_institution(
            institution="anything", user_record="does-not-matter"
        )
        == "anything"
    )
    ...
    successful_result = _get_defaulted_institution(
        institution=None,
...
def test_get_defaulted_project():
    ...
    assert (
        _get_defaulted_project(project=SOME_PROJECT, user_record="does-not-matter")
        == SOME_PROJECT
    )
    assert (
        _get_defaulted_project(project="anything", user_record="does-not-matter")
        == "anything"
    )
    ...
    try:
        _get_defaulted_project(
            project=None, user_record=make_user_record(project_roles=[])
        )
    except Exception as e:
        assert str(e).startswith("Your user profile declares no project")
    else:
    ...
    try:
        ...
        )  # pragma: no cover - we hope never to see this executed

    successful_result = _get_defaulted_project(
        project=None,
        user_record=make_user_record(
            project_roles=[
                {"project": {"@id": "/projects/the_only_project"}, "role": "scientist"}
            ]
        ),
    )
...
def test_ingestion_submission_item_url():
    ...
    assert (
        _ingestion_submission_item_url(server="http://foo.com", uuid="123-4567-890")
...
def os_simulation(*, simulation_mode):
    ...
    assert (
        simulation_mode in OS_SIMULATION_MODES
    ), f"{simulation_mode} is not a defined os simulation mode."
...
class Scenario:
    ...
    def __init__(self, start_time=None, wait_time_delta=None, bundles_bucket=None):
        ...
        self.wait_time_delta = (
            wait_time_delta or self.WAIT_TIME_FOR_TEST_UPDATES_SECONDS
        )
    ...
    def make_uploaded_lines(self):
        ...
        result.append(
            f"{uploaded_time} Metadata bundle uploaded to bucket ({self.bundles_bucket});"
    ...
    def make_wait_lines(self, wait_attempts, outcome: str = None, start_delta: int = 0):
        ...
        adjusted_scenario = Scenario(
            start_time=uploaded_time, wait_time_delta=time_delta_from_start
        )
        wait_time = adjusted_scenario.get_time_after_wait()
        result.append(
            f"{wait_time} Checking ingestion process for IngestionSubmission uuid {SOME_UUID} ..."
        )
        ...
        for idx in range(wait_attempts + 1):
            ...
            wait_time = adjusted_scenario.get_time_after_wait()
            wait_line = (
                f"{ERASE_LINE}{wait_time} Checking processing"
            ...
            if nchecks >= wait_attempts:
                ...
                if outcome == "timeout":
                    wait_line = (
                        f"{ERASE_LINE}{wait_time} Giving up waiting for processing completion"
                        f" | Status: Not Done Yet | Checked: {nchecks + 1} times\n\r"
                    )
                else:
                    wait_line = (
                        f"{ERASE_LINE}{wait_time} Processing complete"
            ...
            for i in range(PROGRESS_CHECK_INTERVAL):
                time_delta_from_start += (
                    2  # Extra 1 for the 1-second sleep loop in utils.check_repeatedly
                )
                adjusted_scenario = Scenario(
                    start_time=uploaded_time, wait_time_delta=time_delta_from_start
                )
    ...
    def make_timeout_lines(cls, *, get_attempts=ATTEMPTS_BEFORE_TIMEOUT):
        ...
        return [
            f"Exiting after check processing timeout"
    ...
    def get_elapsed_time_for_get_attempts(self, get_attempts):
        ...
        extra_waits = 3
        wait_time_delta = (
            (PROGRESS_CHECK_INTERVAL + self.wait_time_delta) * get_attempts
            + PROGRESS_CHECK_INTERVAL
            + extra_waits
        )
        elapsed_time_delta = initial_check_time_delta + wait_time_delta
        adjusted_scenario = Scenario(
            start_time=self.start_time, wait_time_delta=elapsed_time_delta
        )
    ...
    def make_submission_lines(cls, get_attempts, outcome):
        ...
        result += (
            scenario.make_uploaded_lines()
        )  # uses one tick, so we start wait lines offset by 1
        if wait_attempts > 0:
            result += scenario.make_wait_lines(
                wait_attempts, outcome=outcome, start_delta=1
            )
    ...
    def make_timeout_submission_lines(cls):
        ...
        result += scenario.make_wait_lines(
            ATTEMPTS_BEFORE_TIMEOUT - 1, outcome="timeout", start_delta=1
        )
...
def test_resolve_app_args():
    ...
    res = _resolve_app_args(
        consortium=sample_consortium,
        submission_center=sample_submission_center,
        institution=None,
        project=None,
        lab=None,
        award=None,
    ...
    assert res == {
        "consortia": [sample_consortium],
        "submission_centers": [sample_submission_center],
    }

    res = _resolve_app_args(
        consortium=sample_consortia,
        submission_center=sample_submission_centers,
        institution=None,
        project=None,
        lab=None,
        award=None,
    ...
    assert res == {
        "consortia": sample_consortia_list,
        "submission_centers": sample_submission_centers_list,
    }
    ...
    assert (
        str(exc.value)
        == "There are 2 inappropriate arguments: --institution and --project."
    )
...
def test_submit_any_ingestion():
    ...
    def mocked_resolve_app_args(
        *, institution, project, lab, award, consortium, submission_center, app
    ):
        ignored(
            institution, project, award, lab, consortium, submission_center
        )  # not relevant to this mock
    ...
    with mock.patch.object(
        Portal, "key", new_callable=mock.PropertyMock
...
def test_get_defaulted_lab():
    ...
    assert (
        _get_defaulted_lab(lab="anything", user_record="does-not-matter") == "anything"
    )
    ...
    assert (
        _get_defaulted_lab(
            lab=None, user_record=make_user_record(), error_if_none=False
        )
        is None
    )
...
def test_get_defaulted_award():
    ...
    assert (
        _get_defaulted_award(award=SOME_AWARD, user_record="does-not-matter")
        == SOME_AWARD
    )
    assert (
        _get_defaulted_award(award="anything", user_record="does-not-matter")
        == "anything"
    )

    successful_result = _get_defaulted_award(
        award=None,
        user_record=make_user_record(
            lab={
                "@id": SOME_LAB,
    ...
    try:
        _get_defaulted_award(
            award=None, user_record=make_user_record(award_roles=[]), error_if_none=True
        )
    except Exception as e:
    ...
    assert str(exc.value) == (
        "Your lab (/lab/good-lab/) declares multiple awards."
    ...
    assert (
        _get_defaulted_award(
            award=None, user_record=make_user_record(), error_if_none=False
        )
        is None
    )
    ...
    with pytest.raises(Exception) as exc:
        _get_defaulted_award(
            award=None, user_record=make_user_record(), error_if_none=True
        )
...
def test_get_defaulted_consortia():
    ...
    assert (
        _get_defaulted_consortia(
            consortia=SOME_CONSORTIA, user_record="does-not-matter"
        )
        == SOME_CONSORTIA
    )
    assert _get_defaulted_consortia(
        consortia=["anything"], user_record="does-not-matter"
    ) == ["anything"]

    user_record = make_user_record(consortia=[{"@id": SOME_CONSORTIUM}])

    successful_result = _get_defaulted_consortia(
        consortia=None, user_record=user_record
    )
    ...
    with pytest.raises(SystemExit) as exc:
        ...
        _get_defaulted_consortia(
            consortia=None, user_record=make_user_record(), error_if_none=False
        ) == []

    with pytest.raises(Exception) as exc:
        _get_defaulted_consortia(
            consortia=None, user_record=make_user_record(), error_if_none=True
        )
...
def test_get_defaulted_submission_centers():
    ...
    assert (
        _get_defaulted_submission_centers(
            submission_centers=SOME_SUBMISSION_CENTERS, user_record="does-not-matter"
        )
        == SOME_SUBMISSION_CENTERS
    )
    assert _get_defaulted_submission_centers(
        submission_centers=["anything"], user_record="does-not-matter"
    ) == ["anything"]

    user_record = make_user_record(submission_centers=[{"@id": SOME_SUBMISSION_CENTER}])

    successful_result = _get_defaulted_submission_centers(
        submission_centers=None, user_record=user_record
    )
    ...
    with pytest.raises(SystemExit) as exc:
        _get_defaulted_submission_centers(
            submission_centers=None, user_record=make_user_record()
        ) == []
        _get_defaulted_submission_centers(
            submission_centers=None, user_record=make_user_record(), error_if_none=False
        ) == []

    with pytest.raises(Exception) as exc:
        _get_defaulted_submission_centers(
            submission_centers=None, user_record=make_user_record(), error_if_none=True
        )
...
def test_do_app_arg_defaulting():
    ...
    def get_defaulted_foo(
        foo, user_record, error_if_none=False, quiet=False, verbose=False
    ):
        ignored(error_if_none)  # not needed for this mock
        return (
            user_record.get("default-foo", default_default_foo) if foo is None else foo
        )
    ...
    with mock.patch.object(
        submission_module, "APP_ARG_DEFAULTERS", defaulters_for_testing
    ):
        ...
        default_default_foo = (
            None  # make defaulter return None if default not found on the user
        )
        ignorable(
            default_default_foo
        )  # it gets used in the closure, PyCharm should know
...
def test_summarize_submission():
    ...
    # server supplied
    summary = _summarize_submission(
        uuid="some-uuid", server="some-server", app="some-app"
    )
    assert summary == "check-submission --server some-server some-uuid"

    # If both are supplied, env wins.
    summary = _summarize_submission(
        uuid="some-uuid", server="some-server", env="some-env", app="some-app"
    )
    assert summary == "check-submission --env some-env some-uuid"

    # If neither is supplied, well, that shouldn't really happen, but we'll see this:
    summary = _summarize_submission(
        uuid="some-uuid", server=None, env=None, app="some-app"
    )
...
def test_check_ingestion_progress():
    ...
    def test_it(data, *, expect_done, expect_short_status):
        ...
        with mock.patch("dcicutils.portal_utils.Portal.get", mocked_get):
            res = _check_ingestion_progress(
                "some-uuid",
                keypair=("some-keypair", "some-keypair"),
    ...
    test_it(
        {"processing_status": {"progress": "13%"}},
        expect_done=False,
        expect_short_status="13%",
    )
    test_it(
        {"processing_status": {"progress": "working"}},
        expect_done=False,
        expect_short_status="working",
    )
    test_it(
        {"processing_status": {"state": "started", "outcome": "indexed"}},
        expect_done=False,
        expect_short_status=None,
    )
    test_it(
        {"processing_status": {"state": "started"}},
        expect_done=False,
        expect_short_status=None,
    )
    test_it(
        {"processing_status": {"state": "done", "outcome": "indexed"}},
        expect_done=True,
        expect_short_status="indexed",
    )
    test_it(
...
""" Set of simple unit tests for utility/format functions in submission.py
    mostly generated by CHATGPT to cover code paths not otherwise covered."""
# --------------------------------------------------------------------------------------
# _collapse_consecutive_ints
# --------------------------------------------------------------------------------------
...
def test_collapse_consecutive_ints(rows, expected):
    assert _collapse_consecutive_ints(rows) == expected


def test_collapse_consecutive_ints_non_sorted():
    assert _collapse_consecutive_ints(["5", "3", "4"]) == "3-5"
...
# ----------------------------------------------------------------------
# Basic valid collapsing
# ----------------------------------------------------------------------


def test_collapse_single_prefix():
    srcs = [
        "Thing.x [row: 1]",
        "Thing.x [row: 2]",
        "Thing.x [row: 3]",
    ]
    assert _collapse_formatted_srcs(srcs) == [
        "Thing.x [rows: 1-3]",
    ]


def test_collapse_multiple_prefixes():
    srcs = [
        "A.field [row: 1]",
        "B.field [row: 2]",
        "B.field [row: 3]",
    ...
    # grouped independently and sorted by prefix
    assert _collapse_formatted_srcs(srcs) == [
        "A.field [rows: 1, 4]",
        "B.field [rows: 2, 3, 7]",
    ]
...
def test_invalid_entry_is_preserved():
    srcs = [
        "A.field [row: 1]",
        "BAD FORMAT STRING",
        "A.field [row: 2]",
...
def test_invalid_first_valid_after():
    srcs = [
        "XXX BAD",
        "A.field [row: 1]",
        "A.field [row: 2]",
...
# ----------------------------------------------------------------------
# Prefix and ordering behavior
# ----------------------------------------------------------------------


def test_prefixes_sorted_stably():
    srcs = [
        "B.x [row: 3]",
        "A.x [row: 1]",
        "B.x [row: 4]",
...
def test_preserves_invalid_ordering():
    srcs = [
        "INVALID1",
        "A.x [row: 1]",
        "INVALID2",
...
def test_verbose_returns_uncollapsed():
    srcs = [
        "A.x [row: 1]",
        "B.x [row: 2]",
    ]
    assert _collapse_formatted_srcs(srcs, verbose=True) == srcs


def test_verbose_still_returns_invalid_as_is():
    srcs = [
        "GOOD.field [row: 1]",
        "BAD FORMAT",
    ]
    # verbose = True returns *exactly the input list* — this matches original behavior
    assert _collapse_formatted_srcs(srcs, verbose=True) == srcs


# ----------------------------------------------------------------------
# None / empty / trivial cases
# ----------------------------------------------------------------------


def test_empty_srcs():
    assert _collapse_formatted_srcs([]) == []


def test_single_item():
    assert _collapse_formatted_srcs(["A.x [row: 10]"]) == ["A.x [row: 10]"]
...
def test_format_src_row_and_column():
    issue = {"src": {"file": "data.tsv", "column": "foo", "row": 3}}
    # row 3 becomes row 4
    assert _format_src(issue) == "data.foo [row: 4]"


def test_format_src_file_with_gz():
    issue = {"src": {"file": "blah.tsv.gz", "row": 1}}
    assert _format_src(issue) == "blah [row: 2]"


def test_format_src_from_sheet():
    issue = {"src": {"sheet": "SheetA", "row": 0}}
    assert _format_src(issue) == "SheetA"


def test_format_src_warning_fallback():
    issue = {"warning": "Something happened"}
    assert _format_src(issue) == "Warning"


def test_format_src_error_fallback():
    issue = {"error": "Bad input"}
    assert _format_src(issue) == "Error"


def test_format_src_blank():
    assert _format_src({}) == "Issue"
...
def test_format_issue_simple_error():
    issue = {"error": "Validation error at '$.thing': message", "src": {"column": "x"}}
    # _format_issue strips prefixes
    formatted = _format_issue(issue)
    assert "x" in formatted
    assert "message" in formatted
    assert "Validation error" not in formatted
    assert "$." not in formatted


def test_format_issue_warning():
    issue = {"warning": "Careful!", "src": {"column": "foo"}}
    assert _format_issue(issue) == "foo: Careful!"


def test_format_issue_truncated():
    issue = {
        "truncated": True,
        "more": 10,
    ...
    assert "Truncated result set" in out
    assert "More: 10" in out
    assert "See:" in out


def test_format_issue_no_message():
    # Should return empty string
    assert _format_issue({}) == ""


def test_format_issue_src_and_row():
    issue = {
        "src": {"sheet": "S1", "column": "foo", "row": 5},
        "error": "Validation error '$.x': bad",
    }
    formatted = _format_issue(issue)
    assert "S1.foo [row: 6]" in formatted
    assert "bad" in formatted


def test_format_issue_uses_format_src_when_present():
    issue = {"src": {"file": "x.tsv", "row": 2}, "warning": "Hello"}
    assert _format_issue(issue) == "x [row: 3]: Hello"
