...
def test_tissue_external_id_validator_tissue_data_not_list(mock_structured_data):
...
def test_tissue_external_id_validator_tissue_without_donor(mock_structured_data):
    """Test validator when Tissue has no donor property."""
    mock_structured_data.data = {
        "Tissue": [
            {"submitted_id": "NDRI_001", "external_id": "EXT001"}
        ]
    }
    _tissue_external_id_validator(mock_structured_data)
    mock_structured_data.note_validation_error.assert_not_called()
...
def test_tissue_external_id_validator_tissue_without_submitted_id(mock_structured_data):
    """Test validator when Tissue has no submitted_id."""
    mock_structured_data.data = {
        "Tissue": [
            {"donor": "NDRI_DONOR001", "external_id": "EXT001"}
        ]
    }
    _tissue_external_id_validator(mock_structured_data)
    mock_structured_data.note_validation_error.assert_not_called()
...
def test_tissue_external_id_validator_matching_external_ids_ndri_tissue(mock_structured_data):
...
def test_tissue_external_id_validator_matching_external_ids_ndri_donor(mock_structured_data):
...
def test_tissue_external_id_validator_mismatched_external_ids_ndri_tissue(mock_structured_data):
...
def test_tissue_external_id_validator_mismatched_external_ids_ndri_donor(mock_structured_data):
...
def test_tissue_external_id_validator_non_ndri_submission_ignored(mock_structured_data):
...
def test_tissue_external_id_validator_multiple_tissues(mock_structured_data):
    ...
    mock_structured_data.data = {
        ...
        "Donor": [
            ...
            {
                "submitted_id": "NDRI_DONOR002",
                "external_id": "DONOR002"
            },
            {
                "submitted_id": "OTHER_DONOR003",
                "external_id": "DONOR003"
            }
        ]
    }
    _tissue_external_id_validator(mock_structured_data)
...
def test_tissue_external_id_validator_donor_without_external_id(mock_structured_data):
    """Test validator when donor is missing external_id - reports error with None."""
    mock_structured_data.data = {
        "Tissue": [
            {
                "submitted_id": "NDRI_TISSUE001",
...
def test_get_term_info_success(mock_search):
    ...
    mock_search.assert_called_once_with(
        '/search/?type=OntologyTerm&identifier=UBERON:0001234',
        mock_key
    )
...
def test_get_term_info_protocol_without_underscore(mock_search):
    ...
    mock_search.return_value = [
        {
            'identifier': 'UBERON:0001234',
            'valid_protocol_ids': ['OCT', 'PAX_FFPE']
        }
    ]
    mock_key = {'key': 'test_key'}
...
def test_tissue_preservation_type_validator_no_tissue_data(mock_structured_data):
    """Test validator when no Tissue data exists."""
    mock_structured_data.data = {}
...
def test_tissue_preservation_type_validator_tissue_data_not_list(mock_structured_data):
    """Test validator when Tissue data is not a list."""
    mock_structured_data.data = {"Tissue": "not a list"}
    _tissue_preservation_type_validator(mock_structured_data)
    mock_structured_data.note_validation_error.assert_not_called()
...
def test_tissue_preservation_type_validator_tissue_without_uberon_id(mock_structured_data):
...
def test_tissue_preservation_type_validator_tissue_without_preservation_type(mock_get_term, mock_structured_data):
...
@patch('submitr.validators.tissue_validator._get_term_info')
def test_tissue_preservation_type_validator_matching_preservation_type(mock_get_term, mock_structured_data):
...
@patch('submitr.validators.tissue_validator._get_term_info')
def test_tissue_preservation_type_validator_mismatched_preservation_type(mock_get_term, mock_structured_data):
...
@patch('submitr.validators.tissue_validator._get_term_info')
def test_tissue_preservation_type_validator_code_not_in_external_id(mock_get_term, mock_structured_data):
...
@patch('submitr.validators.tissue_validator._get_term_info')
def test_tissue_preservation_type_validator_empty_expected_preservation_type(mock_get_term, mock_structured_data):
...
@patch('submitr.validators.tissue_validator._get_term_info')
def test_tissue_preservation_type_validator_preservation_type_substring_match(mock_get_term, mock_structured_data):
...
@patch('submitr.validators.tissue_validator._get_term_info')
def test_tissue_preservation_type_validator_multiple_tissues_mixed_validation(mock_get_term, mock_structured_data):
    """Test validator with multiple tissues, some valid, some invalid."""
...
@patch('submitr.validators.tissue_validator._get_term_info')
def test_tissue_preservation_type_validator_term_info_caching(mock_get_term, mock_structured_data):
...
@patch('submitr.validators.tissue_validator._get_term_info')
def test_tissue_preservation_type_validator_tissue_without_external_id(mock_get_term, mock_structured_data):
...
@patch('submitr.validators.tissue_validator._get_term_info')
def test_tissue_preservation_type_validator_multiple_codes_in_term_info(mock_get_term, mock_structured_data):
    ...
    _tissue_preservation_type_validator(mock_structured_data)
    # Should error because PAX code is in external_id but preservation_type doesn't match FFPE
    assert mock_structured_data.note_validation_error.call_count == 1
