...
# Validator that reports if any Library items defined in the spreadsheet (StructuredDataSet) are missing the strand property in LibraryPreparation if they are linked to RNA Analyte items
...
def _strand_required(structured_data: StructuredDataSet, **kwargs) -> None:
    ...
    for item in data:
        if _ANALYTE_PROPERTY_NAME in item and (submitted_id := item.get("submitted_id")):
            if isinstance(analytes := [ 
                analyte_item for analyte_item in structured_data.data.get(_ANALTYE_SCHEMA_NAME)
                if analyte_item.get("submitted_id") in item.get(_ANALYTE_PROPERTY_NAME)
            ], list):
                for analyte in analytes:
                    if _RNA_VALUE_NAME in analyte.get(_MOLECULE_PROPERTY_NAME):
                        if _LIBRARY_PREP_PROPERTY_NAME in item:
                            if (library_prep := [
                                lp_item for lp_item in structured_data.data.get(_LIBRARY_PREP_SCHEMA_NAME)
                                if lp_item.get("submitted_id") == item.get(_LIBRARY_PREP_PROPERTY_NAME)
                            ][0]):
